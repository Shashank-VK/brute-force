...
# Agent Orchestrator Rules (2026 Edition)

> This file is the primary "Brain" for all AI environments (Claude, Gemini, Cursor). It forces high-level engineering standards and prevents token waste.

## 🏗 The Architecture Layers

**Layer 1: Deterministic Skills**
- All execution logic lives in `.claude/skills/`.
- Use scripts for heavy tasks (scraping, database migrations, complex UI builds).
- Never "guess" code if a skill script exists.

**Layer 2: The Orchestrator (Decision Making)**
- Prioritize **Planning** over **Coding**.
...
**Layer 3: Subagents (Verification)**
...
- `research`: Use for checking 2026 documentation updates via Valyu/Firecrawl.

## 🛠 Mandatory Engineering Workflow

### 1. The "Pre-Flight" Check
Before writing code, you MUST:
1. Run `/grill-me` to stress-test the logic and find edge cases.
2. Run `write-a-prd` to define the "Source of Truth."
3. Run `prd-to-plan` to break the task into "Tracer Bullets" (vertical slices).

### 2. The Build Loop
- Build one "Tracer Bullet" at a time.
- Use `frontend-design` for all UI/UX (Tailwind, clean spacing, mobile-first).
- **Security Guardrail:** Never use `rm -rf` on system directories. Never hardcode API keys.

### 3. The "Self-Annealing" Loop
- If a script fails: Fix it -> Test it -> Update the `SKILL.md` with the new lesson.
- Always improve the system so the same error never happens twice.

## 📝 Technical Standards
- **Language:** Strict TypeScript (no `any`).
- **Styles:** Tailwind CSS with professional design tokens.
- **State:** Modular and predictable.
- **Docs:** Use the `Docusaurus` skill to auto-generate documentation for every major feature.

## 🛡 Security & Safety
- **Hooks:** Pre-commit hooks must run the Linter.
- **Shannon Rule:** Run a security scan on all new API endpoints or database schemas before shipping.
- **Main Branch:** Never push directly to `main`; always suggest a feature branch.

## 🚀 Active Skills Summary
- **Planning:** `grill-me`, `write-a-prd`, `prd-to-plan`.
- **Optimization:** `context-mode` (Mind Map logic), `grep-ai`.
- **Execution:** `frontend-design`, `webapp-testing`, `video-edit`.
- **Research:** `valyu`, `firecrawl`, `literature-research`.